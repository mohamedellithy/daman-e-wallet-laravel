<template>
    <div class="col-sm-12">
        ...
        <div class="col-sm-12 title-heading">
            <h3 class="section-heading">{{ __('master.Send_Request_Payments') }}</h3>
            <p class="section-subheading">{{ __('master.description_Send_Request_Payments') }}</p>
        </div>
                        ...
                        <div class="info col-md-4">
                            <table class="list others-info">
                                ...
                                <tr v-if="payment.status == 0">
                                    <td class="order-btns">
                                        <a :href="`http://e-wallet.test/payments/${payment.id}/edit`" class="btn btn-default-fill">{{ __('master.Edit_Payment') }} <i class="fas fa-arrow-alt-circle-right"></i></a>
                                        <a @click="CancelPayment(payment.id,index)" class="btn btn-red-fill">{{__('master.Cancel_Payment') }} <i class="fas fa-arrow-alt-circle-right"></i></a>
                                    </td>
                                </tr>
                            </table>
                        </div>
...
export default {
    ...
    created() {
        let self = this;
        this.axios.get('wallet-payments/sent').then(function ({data}){
